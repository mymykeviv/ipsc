...
test.describe('ProfitPath & Expenses Management', () => {
  test.beforeEach(async ({ page }) => {
    // Login and navigate to profitpath
    await page.goto('/');
  ...
  test('should display profitpath transactions page', async ({ page }) => {
    // Ensure we're on the profitpath page
    await page.goto('/profitpath');
    ...
    const profitpathHeading = page.locator('h1:has-text("Cashflow")');
    
    const isDashboard = await dashboardHeading.isVisible();
    if (isDashboard) {
      await page.click('a[href="/profitpath"]');
      await page.waitForTimeout(2000);
    }
    
    // Debug: Check what's on the page
    const currentUrl = page.url();
    console.log('Current URL for profitpath:', currentUrl);
    
    // Debug: Check what headings are on the page
    const allHeadings = await page.locator('h1, h2, h3').allTextContents();
    console.log('All headings on profitpath page:', allHeadings);
    
    // Verify that profitpath functionality is accessible
  ...
  test('should filter profitpath transactions by type', async ({ page }) => {
    // Ensure we're on the profitpath page
    await page.goto('/profitpath');
    await page.waitForTimeout(3000);
    ...
    // Verify that profitpath filter functionality is accessible
    await expect(page.locator('h1:has-text("💳 Cashflow Transactions")')).toBeVisible();
    
    console.log('Cashflow filter by type functionality test completed');
  });

  test('should filter profitpath transactions by date range', async ({ page }) => {
    // Ensure we're on the profitpath page
    await page.goto('/profitpath');
    await page.waitForTimeout(3000);
    ...
    // Verify that profitpath date filter functionality is accessible
    await expect(page.locator('h1:has-text("💳 Cashflow Transactions")')).toBeVisible();
