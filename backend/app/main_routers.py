...
def invoice_pdf(
    ...
    # Get template
    if template_id:
        template = db.query(GSTInvoiceTemplate).filter(GSTInvoiceTemplate.id == template_id).first()
        if not template:
            raise HTTPException(status_code=404, detail='Invoice template not found')
    else:
        # Get default template
        template = db.query(GSTInvoiceTemplate).filter(
            GSTInvoiceTemplate.is_default == True,
            GSTInvoiceTemplate.is_active == True
        ).first()
        if not template:
            raise HTTPException(status_code=404, detail='No default invoice template found')
    ...
    invoice_data = {
        "supplier": {
            ...
            "legal_name": supplier.name if supplier else "N/A",
            "trade_name": "",
            "address": {
                "line1": supplier.billing_address_line1 or "",
                "line2": supplier.billing_address_line2 or "",
        ...
        "invoice": {
            ...
            "place_of_supply": {
                ...
                "state_code": inv.place_of_supply_state_code
            },
            "eway_bill_number": inv.eway_bill_number or "",
            "reverse_charge": inv.reverse_charge,
            "export_supply": inv.export_supply
        },
        "customer": {
            "name": customer.name if customer else "N/A",
            "address": {
                "line1": customer.billing_address_line1 or "",
                "line2": customer.billing_address_line2 or "",
                "city": customer.billing_city or "",
                "state": customer.billing_state or "",
                "pin": customer.billing_pincode or ""
            },
            "gstin": customer.gstin if customer else "",
            "email": customer.email if customer else "",
            "phone": customer.contact_number if customer else ""
        },
        "ship_to": {
            "name": customer.name if customer else "N/A",
            "use_bill_to": True,
            "address": {
                "line1": customer.shipping_address_line1 or customer.billing_address_line1 or "",
                "line2": customer.shipping_address_line2 or customer.billing_address_line2 or "",
                "city": customer.shipping_city or customer.billing_city or "",
                "state": customer.shipping_state or customer.billing_state or "",
                "pin": customer.shipping_pincode or customer.billing_pincode or ""
            },
            "gstin": customer.gstin if customer else "",
            "email": customer.email if customer else "",
            "phone": customer.contact_number if customer else ""
    ...
    for item in items:
        ...
        invoice_data["items"].append({
            "sl": len(invoice_data["items"]) + 1,
            "description": item.description or (product.name if product else "N/A"),
            "hsn": item.hsn_code or (product.hsn if product else ""),
            "quantity": item.qty,
    ...
    # Add charges, notes, declaration, signatory
    invoice_data["charges"] = []
    invoice_data["notes"] = inv.notes or ""
    invoice_data["declaration"] = "This is a computer generated invoice and does not require signature."
...
def create_purchase(payload: PurchaseCreate, _: User = Depends(get_current_user), db: Session = Depends(get_db)):
    vendor = db.query(Party).filter(Party.id == payload.vendor_id, Party.is_vendor == True).first()
    if not vendor:
...
def list_purchases(
    ...
    query = db.query(Purchase).join(Party).filter(Party.is_vendor == True)
...
def create_party(payload: PartyCreate, _: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        ...
        # Convert payload to dict and handle type field
        party_data = payload.model_dump()
        party_type = party_data.pop('type', 'customer')  # Remove type field and default to customer
        
        # Set the appropriate boolean flags based on type
        if party_type == 'vendor':
            party_data['is_vendor'] = True
            party_data['is_customer'] = False
        else:  # customer
            party_data['is_customer'] = True
            party_data['is_vendor'] = False
...
# GST Invoice Template Management - API Endpoints

class GSTInvoiceTemplateOut(BaseModel):
    id: int
    template_id: str
    name: str
    description: str | None
...
@api.get('/gst-invoice-templates', response_model=list[GSTInvoiceTemplateOut])
def get_gst_invoice_templates(_: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """Get all GST invoice templates"""
    from .template_configs import get_all_templates
    
    # Check if templates exist in database, if not create them
    existing_templates = db.query(GSTInvoiceTemplate).count()
    ...
    templates = db.query(GSTInvoiceTemplate).filter(
        GSTInvoiceTemplate.is_active == True
    ).order_by(GSTInvoiceTemplate.sort_order).all()
    return templates


@api.get('/gst-invoice-templates/default', response_model=GSTInvoiceTemplateOut)
def get_default_gst_invoice_template(_: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """Get the default GST invoice template"""
    template = db.query(GSTInvoiceTemplate).filter(
        GSTInvoiceTemplate.is_default == True,
        GSTInvoiceTemplate.is_active == True
    ).first()
    ...
    if not template:
        # Get first template as default
        template = db.query(GSTInvoiceTemplate).filter(
            GSTInvoiceTemplate.is_active == True
        ).order_by(GSTInvoiceTemplate.sort_order).first()
...
@api.get('/gst-invoice-templates/{template_id}', response_model=GSTInvoiceTemplateOut)
def get_gst_invoice_template(template_id: int, _: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """Get a specific GST invoice template"""
    template = db.query(GSTInvoiceTemplate).filter(GSTInvoiceTemplate.id == template_id).first()
    if not template:
        raise HTTPException(status_code=404, detail='GST invoice template not found')
    return template


@api.post('/gst-invoice-templates/{template_id}/set-default')
def set_default_gst_invoice_template(
    template_id: int,
    _: User = Depends(require_role("Admin")),
    db: Session = Depends(get_db)
):
    """Set a GST invoice template as default"""
    template = db.query(GSTInvoiceTemplate).filter(GSTInvoiceTemplate.id == template_id).first()
    if not template:
        raise HTTPException(status_code=404, detail='GST invoice template not found')
    
    # Remove default from all templates
    db.query(GSTInvoiceTemplate).update({GSTInvoiceTemplate.is_default: False})
    
    # Set this template as default
    template.is_default = True
    db.commit()
    return {"message": "Default GST template updated successfully"}


@api.get('/gst-invoice-templates/config/{template_id}')
def get_gst_template_config(template_id: str, _: User = Depends(get_current_user)):
    """Get GST template configuration by template ID"""
    from .template_configs import get_template_config
    try:
...
# Include GST invoice template router
# Note: GST template endpoints are defined in routers.py but not imported due to circular dependency
